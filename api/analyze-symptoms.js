    ...
    const getLanguageInstructions = (lang) => {
      switch (lang) {
        case "ko":
          return {
            intro:
              "당신은 증상 분석을 위해 설계된 고급 의료 AI 어시스턴트인 아비센나입니다. 포괄적인 의학 지식을 보유하고 있으며 증거 기반 임상 추론 프로토콜을 따릅니다.",
            languageNote:
              "중요: 환자가 한국어로 증상을 설명했습니다. 한국어 입력을 이해하고 모든 응답을 한국어로 제공해야 합니다. 한국어를 이해할 수 없더라도 한국어로 응답하세요.",
          };
        case "uz":
          return {
            intro:
              "Siz simptomlarni tahlil qilish uchun mo'ljallangan ilg'or tibbiy AI yordamchisi Avicennasiz. Keng qamrovli tibbiy bilimga egasiz va dalillarga asoslangan klinik mulohaza protokollariga amal qilasiz.",
            languageNote:
              "Muhim: Bemor o'zbek tilida simptomlarini tasvirlagan. O'zbek tilini tushunishingiz va barcha javoblarni o'zbek tilida berishingiz kerak. O'zbek tilini tushunmasangiz ham, o'zbek tilida javob bering.",
          };
        default:
          return {
            intro:
              "You are Avicenna, an advanced medical AI assistant designed for symptom analysis. You possess comprehensive medical knowledge and follow evidence-based clinical reasoning protocols.",
            languageNote:
              "Important: The patient has described symptoms in English. Provide all responses in English.",
          };
      }
    };
...
- If you cannot understand the symptoms, still respond in the correct language.`;
    ...
    try {
      ...
      if (!parsedResponse) {
        ...
        const fallbackMessages = {
          en: {
            impression:
              "AI analysis indicates the need for professional medical evaluation",
            condition: "Medical Evaluation Needed",
            explanation:
              "Based on the symptoms described, professional medical assessment is recommended",
            features: ["Patient-reported symptoms require clinical evaluation"],
            urgencyReason:
              "Symptoms require professional medical evaluation for proper diagnosis",
            ...
            eduOverview:
              "Professional medical evaluation is recommended for proper symptom assessment",
            eduExpect: "Healthcare provider will perform thorough evaluation",
            eduPrevention: "Follow medical advice for prevention",
            disclaimer:
              "This AI analysis is for informational purposes only and does not replace professional medical diagnosis or treatment. Seek immediate medical attention for severe symptoms or if condition worsens.",
          },
          ko: {
            impression:
              "AI 분석 결과 전문적인 의학적 평가가 필요함을 시사합니다",
            condition: "의학적 평가 필요",
            explanation:
              "제시된 증상을 바탕으로 전문적인 의학적 평가가 권장됩니다",
            features: ["환자가 보고한 증상은 임상적 평가가 필요합니다"],
            urgencyReason:
              "정확한 진단을 위해 증상에 대한 전문적인 의학적 평가가 필요합니다",
            ...
            eduOverview:
              "정확한 증상 평가를 위해 전문적인 의학적 평가가 권장됩니다",
            eduExpect: "의료 서비스 제공자가 철저한 평가를 수행할 것입니다",
            eduPrevention: "예방을 위해 의학적 조언을 따르십시오",
            disclaimer:
              "이 AI 분석은 정보 제공 목적으로만 제공되며 전문적인 의학적 진단이나 치료를 대체하지 않습니다. 심각한 증상이 있거나 상태가 악화될 경우 즉시 의료 지원을 받으십시오.",
          },
          uz: {
            impression:
              "AI tahlili professional tibbiy baholash zarurligini ko'rsatadi",
            condition: "Tibbiy baholash talab etiladi",
            explanation:
              "Taqdim etilgan alomatlarga asoslanib, professional tibbiy baholash tavsiya etiladi",
            features: [
              "Bemor tomonidan bildirilgan alomatlar klinik baholashni talab qiladi",
            ],
            urgencyReason:
              "To'g'ri tashxis qo'yish uchun alomatlar professional tibbiy baholashni talab qiladi",
            redFlags: [
              "Alomatlarning yomonlashishi",
              "Yangi tashvishli alomatlar",
            ],
            timeframe:
              "24-48 soat ichida yoki alomatlar yomonlashsa, undan ham ertaroq",
            immediate: ["Alomatlarni diqqat bilan kuzatib boring"],
            monitoring: [
              "Alomatlarning yomonlashishi yoki yangi alomatlarga e'tibor bering",
            ],
            ...
            eduOverview:
              "To'g'ri alomatlarni baholash uchun professional tibbiy ko'rikdan o'tish tavsiya etiladi",
            eduExpect:
              "Tibbiy yordam ko'rsatuvchi provayder to'liq baholashni amalga oshiradi",
            eduPrevention:
              "Oldini olish uchun tibbiy maslahatlarga rioya qiling",
            disclaimer:
              "Ushbu AI tahlili faqat ma'lumot berish uchun mo'ljallangan va professional tibbiy diagnostika yoki davolanishni o'rnini bosmaydi. Qattiq alomatlar yoki ahvol yomonlashgan taqdirda darhol tibbiy yordamga murojaat qiling.",
          },
        };
    ...
    if (
      !parsedResponse.primaryAnalysis ||
      !parsedResponse.urgencyAssessment ||
        ...
        differentialDiagnosis: parsedResponse.differentialDiagnosis || [
          {
            ...
            likelihood: "moderate",
            explanation:
              "Based on your symptoms, it's recommended to consult with a healthcare professional",
    ...
    if (
      ...
      parsedResponse.recommendations.followUp
    ) {
    ...
    if (error.name === "AbortError") {
      return res.status(504).json({
        ...
        message:
          'The AI analysis took longer than expected. This sometimes happens with complex or informal symptom descriptions. Please try rephrasing your symptoms more clearly (e.g., "tailbone pain after falling" instead of "fell on my butt") or try again.',
        timestamp: new Date().toISOString(),
      });
    }
      ...
      return res.status(503).json({
        ...
        fallback: {
          primaryAnalysis: {
            ...
            clinicalImpression:
              "Professional medical evaluation recommended during service interruption",
          },
          differentialDiagnosis: [
            {
              condition: "General Health Concern",
              likelihood: "moderate",
              explanation:
                "Based on your symptoms, it's recommended to monitor your condition and consult with a healthcare professional if symptoms persist or worsen",
          ...
          urgencyAssessment: {
            level: "see_doctor_soon",
            reasoning:
              "Given service limitations, professional evaluation is recommended for proper assessment",
            redFlags: [
              "Severe symptoms",
          ...
          specialistReferral: {
            ...
            specialty: "General Practitioner",
            reasoning:
              "Start with primary care evaluation for initial assessment",
            specificConditions: ["General health concerns"],
            whenToSee: "within 1 week",
          },
          educationalContent: {
            overview:
              "Professional medical evaluation is recommended for proper symptom assessment",
            whatToExpected:
              "Healthcare provider will perform comprehensive evaluation",
            prevention:
              "Regular health monitoring and early medical consultation for concerning symptoms",
          },
          disclaimer:
            "This AI analysis is for informational purposes only and does not replace professional medical diagnosis or treatment. Seek immediate medical attention for severe symptoms or if condition worsens.",
        },
